...
type FlatQuoteField = Exclude<keyof QuoteFormValues, "extraDrivers" | "extraVehicles">;
...
export default function GetAutoQuotePage() {
  ...
  const {
    fields: extraDriverFields,
    append: appendDriver,
    remove: removeDriver,
  } = useFieldArray({ control, name: "extraDrivers" });

  const {
    fields: extraVehicleFields,
    append: appendVehicle,
    remove: removeVehicle,
  } = useFieldArray({ control, name: "extraVehicles" });
  ...
  const fieldTooltips: Partial<Record<FlatQuoteField, string>> = {
  ...
  const renderInfoIcon = (name: FlatQuoteField) => {
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="mt-8 space-y-6">
                          ...
                          {extraVehicleFields.map((field, index) => (
                            <div
                              key={field.id}
                              className="relative rounded-lg border border-border p-4 md:p-5 space-y-4"
                            >
                              ...
                              <div className="grid md:grid-cols-2 gap-5">
                                <div className="space-y-2">
                                  ...
                                  <Input
                                    ...
                                    placeholder="VIN"
                                    className={cn(
                                      errors.extraVehicles?.[index]?.vinNumber &&
                                        "border-red-500",
                                    )}
                                    {...register(`extraVehicles.${index}.vinNumber` as const)}
                                  />
                                  {errors.extraVehicles?.[index]?.vinNumber && (
                                    <p className="text-xs text-red-500">
                                      {errors.extraVehicles[index]?.vinNumber?.message}
                                    </p>
                                  )}
                                </div>
                                <div className="space-y-2">
                                  <Label className="text-sm font-medium">Vehicle use *</Label>
                                  <Select
                                    value={watch(`extraVehicles.${index}.vehicleUse`) || ""}
                                    onValueChange={(val) =>
                                      setValue(
                                        `extraVehicles.${index}.vehicleUse`,
                                        val,
                                  ...
                                  >
                                    <SelectTrigger
                                      className={cn(
                                        errors.extraVehicles?.[index]?.vehicleUse &&
                                          "border-red-500",
                                      )}
                                    >
                                ...
                                <div className="space-y-2 md:col-span-2">
                                  ...
                                  <Select
                                    value={
                                      watch(
                                        `extraVehicles.${index}.estimatedAnnualMileage`,
                                      ) || ""
                                    }
                                    onValueChange={(val) =>
                                      setValue(
                                  ...
                                  >
                                    <SelectTrigger
                                      className={cn(
                                        errors.extraVehicles?.[index]
                                          ?.estimatedAnnualMileage && "border-red-500",
                                      )}
                                    >
                                      <SelectValue placeholder="Select range" />
                                    </SelectTrigger>
                                    <SelectContent>
                                      {["1-5K", "6-10K", "11-15K", "15K+"].map((item) => (
                                        <SelectItem key={item} value={item}>
                                          {item}
                                        </SelectItem>
                                      ))}
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-2 mb-6">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                              Add everyone who will be listed on the auto policy
                              (not the primary applicant above).
                            </p>
                          </div>
                          <Button
                            type="button"
                            variant="outline"
                            size="sm"
                            onClick={() => appendDriver(emptyDriver)}
                          >
                            <Plus className="h-4 w-4 mr-1" /> Add driver
                          </Button>
                        </div>
                        {extraDriverFields.length === 0 && (
                          <p className="text-sm text-muted-foreground mb-4">
                            No additional drivers. Use &quot;Add driver&quot; if
                            you need to list more than the primary.
                          </p>
                        )}
                        {extraDriverFields.map((field, index) => (
                          <div
                            key={field.id}
                            className="relative rounded-lg border border-border p-4 md:p-5 mb-6"
                          >
                            <div className="flex items-center justify-between mb-4">
                              <p className="text-sm font-semibold text-foreground">
                                Driver {index + 2} (additional)
                              </p>
                              <Button
                                type="button"
                                variant="ghost"
                                ...
                                aria-label={`Remove additional driver ${index + 1}`}
                              >
                                <Trash2 className="h-4 w-4" />
                              </Button>
                            </div>
                            <div className="grid md:grid-cols-2 gap-5">
                              <div className="space-y-2">
                                ...
                                <Input
                                  className={cn(
                                    errors.extraDrivers?.[index]?.firstName &&
                                      "border-red-500",
                                  )}
                                  {...register(
                                    `extraDrivers.${index}.firstName` as const,
                                  )}
                                />
                                {errors.extraDrivers?.[index]?.firstName && (
                                  <p className="text-xs text-red-500">
                                    {errors.extraDrivers[index]?.firstName?.message}
                                  </p>
                                )}
                              </div>
                              <div className="space-y-2">
                                ...
                                <Input
                                  className={cn(
                                    errors.extraDrivers?.[index]?.lastName &&
                                      "border-red-500",
                                  )}
                                  {...register(
                                    `extraDrivers.${index}.lastName` as const,
                                  )}
                                />
                                {errors.extraDrivers?.[index]?.lastName && (
                                  <p className="text-xs text-red-500">
                                    {errors.extraDrivers[index]?.lastName?.message}
                                  </p>
                                )}
                              </div>
                              <div className="space-y-2">
                                ...
                                <Input
                                  placeholder="MM/DD/YYYY"
                                  className={cn(
                                    errors.extraDrivers?.[index]?.dateOfBirth &&
                                      "border-red-500",
                                  )}
                                  {...register(
                                    `extraDrivers.${index}.dateOfBirth` as const,
                                  )}
                                />
                                {errors.extraDrivers?.[index]?.dateOfBirth && (
                                  <p className="text-xs text-red-500">
                                    {errors.extraDrivers[index]?.dateOfBirth?.message}
                                  </p>
                                )}
                              </div>
                              <div className="space-y-2">
                                ...
                                <Input
                                  className={cn(
                                    errors.extraDrivers?.[index]
                                      ?.driverLicenseNumber && "border-red-500",
                                  )}
                                  {...register(
                                    `extraDrivers.${index}.driverLicenseNumber` as const,
                                  )}
                                />
                                {errors.extraDrivers?.[index]?.driverLicenseNumber && (
                                  <p className="text-xs text-red-500">
                                    {
                                      errors.extraDrivers[index]?.driverLicenseNumber
                                        ?.message
                              ...
                              <div className="space-y-2">
                                ...
                                <Select
                                  value={watch(`extraDrivers.${index}.maritalStatus`) || ""}
                                  onValueChange={(val) =>
                                    setValue(
                                      `extraDrivers.${index}.maritalStatus`,
                                      val,
                                      { shouldValidate: true, shouldDirty: true },
                                    )
                                  }
                                >
                                  <SelectTrigger
                                    className={cn(
                                      errors.extraDrivers?.[index]?.maritalStatus &&
                                        "border-red-500",
                                    )}
                                  >
                              ...
                              <div className="space-y-2">
                                <Label>Gender *</Label>
                                <Select
                                  value={watch(`extraDrivers.${index}.gender`) || ""}
                                  onValueChange={(val) =>
                                    setValue(
                                      `extraDrivers.${index}.gender`,
                                      val,
                                      { shouldValidate: true, shouldDirty: true },
                                    )
                                  }
                                >
                                  <SelectTrigger
                                    className={cn(
                                      errors.extraDrivers?.[index]?.gender && "border-red-500",
                                    )}
                                  >
                                    <SelectValue placeholder="Select" />
                                  </SelectTrigger>
                                  <SelectContent>
                                    {["Female", "Male", "Non-specified"].map((item) => (
                                      <SelectItem key={item} value={item}>
                                        {item}
                                      </SelectItem>
                                    ))}
